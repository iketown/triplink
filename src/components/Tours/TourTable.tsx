...
export const TourTable = ({ tour }: ITourTable) => {
  ...
  const data = useMemo(() => {
    ...
    const dates = getArrayOfDates({ first: firstDate, last: lastDate }).map(date => moment(date).format('YYYY-MM-DD'))
    ...
    <div style={{ maxWidth: '100%' }}>
      {data && <MaterialTable
        ...
        columns={[
          ...
          { title: 'Events', render: (data) => <TableEventRender events={data.tableEvents} /> },
          { title: 'Flights', render: (data) => <TableFlightRender date={data.date} tourId={tour.id} flights={data.flights} /> },
          { title: 'Hotels' },
        ]}
        //@ts-ignore
        data={data}
        ...
        // ]}
        title={<div>
          <Typography variant='subtitle2'>{tour.name.toUpperCase()}</Typography>
          <Typography color='textSecondary' variant='caption' >
            {`${moment(tour.startDate).format('MMM D')} - ${moment(tour.endDate).format('MMM D, YYYY')}`}
          </Typography>
        </div>}
        options={
          {
...
const TableEventRender = ({ events }: { events: TourEvent[] }) => {
  ...
  return <div>
    {events
      .map((evt, index, arr) => {
        if (index !== arr.findIndex(e => e.locBasic.placeId === evt.locBasic.placeId)) return null
        const showsLength = arr.filter(e => e.locBasic.placeId === evt.locBasic.placeId).length
        return (
          <Chip key={evt.id} avatar={<StarBorder />} onClick={() => alert('hey now')} variant='outlined'
            label={`${evt.locBasic.locShortName} ${showsLength > 1 ? showsLength : ''}`} />
        )
      })}
  </div>
}

const TableFlightRender = ({ flights, date, tourId }: { flights: any[], date: string, tourId: string }) => {
